...
export function generateLivePostFromCSV(gov: GovernorateCode): SocialPost {
  ...
  // Believable comments
  const commentPool = [
    { id: 'ca', username: 'hasan_bagh', textAr: '????? ????? ???? ????????? ?? ??? ?', textKu: '?????? ??? ??? ?????? ??? ?????', textEn: 'Excellent service, highly recommended!' },
    { id: 'cb', username: 'sara_kurd', textAr: '???? ??????? ?????? ??????? ??????? ?????', textKu: '??? ????? ???????? ?????? ????????? ??', textEn: 'Beautiful setup and lovely customer support!' },
    { id: 'cc', username: 'ali_iraqi', textAr: '???? ??????? ??????? ????? ???????? ???? ??????', textKu: '???? ???? ????? ??? ?????? ???', textEn: 'Will definitely visit with my family this week.' },
    { id: 'cd', username: 'dr_noor', textAr: '???? ???????? ???? ?????? ??? ?????? ??', textKu: '????????? ?? ?????????????? ?????? ????', textEn: 'Superb! Legit and very smooth interaction.' }
  ];
  ...
  const contactLineAr = baseBiz.phone 
    ? `\n\n?? ??????? ??????????: ${baseBiz.phone} � ??????: ${govLabel.ar}? ${baseBiz.name.ar}`
    : `\n\n?? ??????: ${govLabel.ar}? ${baseBiz.name.ar}`;

  const contactLineKu = baseBiz.phone 
    ? `\n\n?? ?? ???????? ? ???????: ${baseBiz.phone} � ????????: ${govLabel.ku}`
    : `\n\n?? ????????: ${govLabel.ku}`;
  ...
  return {
    ...
    businessName: `${baseBiz.name.ar} - ${govLabel?.ar || '??????'}`,
