...
export default function AddBusinessForm({ 
  ...
  const myBusiness =
    (ownedBusinessId ? businesses.find((b) => b.id === ownedBusinessId) : undefined) ||
    businesses.find((b) => b.ownerUid === user?.uid || (userProfile?.businessId && b.id === userProfile.businessId));
  ...
  // OWNER ROLE but no business linked yet (new claim system)
    ...
    return (
      <div className="w-full max-w-xl mx-auto bg-white/5 border border-white/10 rounded-[28px] p-8 text-center space-y-6 relative overflow-hidden backdrop-blur-md shadow-2xl">
        ...
        <div className="relative space-y-4">
          ...
          <div>
            <span className="text-[10px] text-luxury-gold font-extrabold uppercase tracking-widest block font-mono">
              {currentLang === 'en' ? 'Business Owner Portal' : currentLang === 'ku' ? 'پۆرتاڵی خاوەن کار' : 'بوابة صاحب العمل'}
            </span>
            <h3 className="text-base font-black text-white mt-1">
              {currentLang === 'en' ? 'Claim your existing business' : currentLang === 'ku' ? 'کارەکەت بەخۆت بکە' : 'طالب بعملك الموجود'}
            </h3>
            <p className="text-xs text-zinc-400 mt-1.5 leading-relaxed max-w-sm mx-auto">
              {currentLang === 'en'
                ? 'Your business already exists in our database. Verify by phone and unlock your dashboard in under 60 seconds.'
                : currentLang === 'ku'
                ? 'کارەکەت لە بنکەدراوەدا هەیە. بە ژمارە پشتڕاست بکە و داشبۆرد بکەرەوە.'
                : 'عملك موجود مسبقاً في قاعدة البيانات. أكد برقم الهاتف وافتح لوحة التحكم بسرعة.'}
            </p>
          </div>

          <button
            onClick={() => onOpenOwnerClaim?.()}
            className="inline-flex items-center gap-2 px-6 py-3.5 bg-gradient-to-r from-luxury-teal to-luxury-gold hover:opacity-90 text-white font-black text-xs uppercase tracking-wider rounded-xl transition cursor-pointer shadow-xl duration-300 font-sans"
            style={{ width: 'auto' }}
          >
            🏢 {currentLang === 'en' ? 'Claim my business' : currentLang === 'ku' ? 'کارەکەم بکەرەوە' : 'طالب بعملي'}
          </button>
        </div>
      </div>
    );
