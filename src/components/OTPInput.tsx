...
export default function OTPInput({
    ...
    if (code.length < 6) {
      setErrorText(
        currentLang === 'en' 
          ...
          : currentLang === 'ku'
          ? 'ØªÚ©Ø§ÛŒÛ• Ú©Û†Ø¯ÛŒ Ù¦ Ú˜Ù…Ø§Ø±Û•ÛŒÛŒ Ø¨Û•Ú•ÛŽÚ©ÛŒ Ø¨Ù†ÙˆÙˆØ³Û•'
  ...
  const t = {
    en: {
      ...
      submitBtn: 'Verify Code & Unlock Control ðŸ”“',
      codeAlert: 'SIMULATION SANDBOX PASSCODE IS:',
    },
    ar: {
      header: 'Ø£Ø¯Ø®Ù„ Ø±Ù…Ø² Ø§Ù„ØªØ­Ù‚Ù‚ Ø§Ù„Ø£Ù…Ù†ÙŠ',
      sub: `Ù„Ù‚Ø¯ Ø£Ø±Ø³Ù„Ù†Ø§ Ø±Ù…Ø² Ù…ÙˆØ§ÙÙ‚Ø© Ø¥Ø«Ø¨Ø§Øª Ø§Ù„Ù…Ù„ÙƒÙŠØ© Ø§Ù„Ù…ÙƒÙˆÙ† Ù…Ù† 6 Ø£Ø±Ù‚Ø§Ù… Ø¥Ù„Ù‰ ${phoneWithCountry} Ø¹Ø¨Ø± Ø´Ø±ÙŠØ­Ø© ÙˆØ§ØªØ³Ø§Ø¨/Ø±Ø³Ø§Ø¦Ù„ Ø§Ù„Ù†ÙˆØ§ÙØ°.`,
      label: 'ØªØ£ÙƒÙŠØ¯ Ø£Ù…Ù†ÙŠ',
      resendCode: 'Ø¥Ø¹Ø§Ø¯Ø© Ø¥Ø±Ø³Ø§Ù„ Ø±Ù…Ø² Ø§Ù„ØªØ­Ù‚Ù‚ Ø§Ù„ØªØ¬Ø±ÙŠØ¨ÙŠ',
      changePhone: 'ØªØºÙŠÙŠØ± Ø±Ù‚Ù… Ø§Ù„Ù‡Ø§ØªÙ',
      indicator: 'Ù‚Ù†Ø§Ø© Ø§Ù„ØªØ­Ù‚Ù‚ Ø§Ù„Ù…Ø´ÙØ±Ø© Ù†Ø´Ø·Ø©',
      submitBtn: 'ØªØ£ÙƒÙŠØ¯ Ø§Ù„Ø±Ù…Ø² ÙˆÙØªØ­ Ù„ÙˆØ­Ø© Ø§Ù„ØªØ­ÙƒÙ… ðŸ”“',
      codeAlert: 'Ø±Ù…Ø² Ø§Ù„ØªØ­Ù‚Ù‚ Ø§Ù„ØªØ¬Ø±ÙŠØ¨ÙŠ Ø§Ù„ÙØ¹Ø§Ù„ Ù‡Ùˆ:',
    },
    ku: {
      header: 'Ú©Û†Ø¯ÛŒ Ù¾Ø´ØªÚ•Ø§Ø³ØªÚ©Ø±Ø¯Ù†Û•ÙˆÛ• Ø¨Ù†ÙˆÙˆØ³Û•',
      sub: `Ø¦ÛŽÙ…Û• Ú©Û†Ø¯ÛŒ Ø¯ÚµÙ†ÛŒØ§Ø¨ÙˆÙˆÙ†Û•ÙˆÛ•ÛŒ Ù¦ Ú˜Ù…Ø§Ø±Û•ÛŒÛŒÙ…Ø§Ù† Ù†Ø§Ø±Ø¯ Ø¨Û† ${phoneWithCountry} Ù„Û• Ú•ÛŽÚ¯Û•ÛŒ Ù†Ø§Ù…Û•ÛŒ Ù…Û†Ø¨Ø§ÛŒÙ„Û•ÙˆÛ•.`,
      label: 'Ù¾Ø´ØªÚ•Ø§Ø³ØªÚ©Ø±Ø¯Ù†Û•ÙˆÛ•ÛŒ Ù¾Ø§Ø±Ø§Ø³ØªÙ†',
      resendCode: 'Ø¯ÙˆØ¨Ø§Ø±Û• Ù†Ø§Ø±Ø¯Ù†Û•ÙˆÛ•ÛŒ Ú©Û†Ø¯',
      changePhone: 'Ú¯Û†Ú•ÛŒÙ†ÛŒ Ú˜Ù…Ø§Ø±Û•ÛŒ Ù…Û†Ø¨Ø§ÛŒÙ„',
      indicator: 'Ù‡ÛŽÚµÛŒ Ø¯ÚµÙ†ÛŒØ§Ø¨ÙˆÙˆÙ†Û•ÙˆÛ•ÛŒ Ù¾Ø§Ø±ÛŽØ²Ø±Ø§Ùˆ Ú†Ø§Ù„Ø§Ú©Û•',
      submitBtn: 'Ù¾Ø´ØªÚ•Ø§Ø³ØªÚ©Ø±Ø¯Ù†Û•ÙˆÛ•ÛŒ Ú©Û†Ø¯ Ùˆ Ú†ÙˆÙˆÙ†Û•Ú˜ÙˆÙˆØ±Û•ÙˆÛ• ðŸ”“',
      codeAlert: 'Ú©Û†Ø¯ÛŒ ØªØ§Ù‚ÛŒÚ©Ø±Ø¯Ù†Û•ÙˆÛ•ÛŒ Ø³Û•Ø±Ú©Û•ÙˆØªÙˆÙˆ Ø¨Ø±ÛŒØªÛŒÛŒÛ• Ù„Û•:',
    }
  }[currentLang];
