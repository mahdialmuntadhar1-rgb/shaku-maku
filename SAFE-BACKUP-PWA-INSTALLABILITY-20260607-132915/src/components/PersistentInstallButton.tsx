...
const NOT_READY_TEXT: Record<Lang, string> = {
  ar: 'إذا لم تظهر نافذة التثبيت، افتح الموقع من Chrome على الرابط الرسمي HTTPS ثم اضغط الزر مرة أخرى.',
  ku: 'ئەگەر پەنجەرەی دابەزاندن دەرنەکەوت، ماڵپەڕەکە لە Chrome بە لینکی HTTPS بکەرەوە و دووبارە دایبگرە.',
  en: 'If the install window does not appear, open the HTTPS site in Chrome and tap again.',
};
...
export default function PersistentInstallButton() {
  ...
  useEffect(() => {
    ...
    const handleBeforeInstallPrompt = (event: Event) => {
      ...
      window.__shakuDeferredInstallPrompt = event as BeforeInstallPromptEventLike;
      window.__shakuInstallReady = true;
    ...
    window.addEventListener('shaku-app-installed', handleInstalled);
    ...
    return () => {
      ...
      window.removeEventListener('shaku-app-installed', handleInstalled);
  ...
  async function triggerInstall(event?: React.SyntheticEvent<HTMLButtonElement>) {
    event?.preventDefault();
    ...
    if (!promptEvent) {
      setNote(NOT_READY_TEXT[lang]);
      window.setTimeout(() => setNote(''), 4200);
      return;
    }

    try {
      // Keep prompt() directly inside the tap/click handler.
      promptEvent.prompt();
      ...
      <style>
        {`
          ...
          .shaku-pwa-install-button {
            ...
            min-width: ${lang === 'ku' ? '104px' : '78px'} !important;
            font-size: ${lang === 'ku' ? '12px' : '12.5px'} !important;
