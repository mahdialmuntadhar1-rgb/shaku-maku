...
>>>>>>> 59cf695 (freeze: stable login checkpoint 20260602-180026)
...
=======
      ...
      if (user) {
        writeSession(token || session_, user);
        return { success: true, ...response.data, user, token: token || session_ };
      }
      ...
      return {
        success: false,
        error: axios.isAxiosError(error) ? error.response?.data?.error || response?.data?.message || 'Login failed' : 'Login failed'
      };
    }
  },
